...
import { BOT_LABELS, type CardT, type GameEvent, type PlayerView, type RoomView } from "@/lib/types";
...
export default function GameTable({ socket, view }: { socket: RoomSocket; view: RoomView }) {
        ...
        <div>
          ...
          {player.bot ? (
            ...
            <p className="text-sm text-ivory-dim/80">
              {profile.data.games_played} parties · {profile.data.games_won} gagnées ·{" "}
              {profile.data.games_played > 0
                ? Math.round((profile.data.games_won / profile.data.games_played) * 100)
                : 0}
              % de victoires
            </p>
          ) : (
...
function Results({ view, socket }: { view: RoomView; socket: RoomSocket }) {
