...
AfterMove = Callable[[Room, list[dict]], Awaitable[None]]
...
def humans_first(room: Room) -> None:
    ...
    k = next((i for i, s in enumerate(room.seats) if s.bot is None), None)
    if not k:
        return
    room.seats[:] = room.seats[k:] + room.seats[:k]
...
def observe(room: Room, events: list[dict]) -> None:
    ...
    if room.bot_memory is None or room.bot_memory.n != n or started:
        room.bot_memory = botbrain.Memory(n)
    room.bot_memory.on_events(events, botbrain.counts13(c.value for c in room.state.pile))
...
async def _choose_hard(room: Room, view: dict) -> tuple[int, int, Comparator | None]:
    ...
    seat = view["your_seat"]
    mem = room.bot_memory or botbrain.Memory(len(view["players"]))
    if HARD_SEARCH:
